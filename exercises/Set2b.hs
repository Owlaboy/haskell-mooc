...
oddFactorial i = 
    if even i
        then oddFactorial' (i-1)
    else oddFactorial' i

oddFactorial' :: Integer -> Integer
oddFactorial' 1 = 1
oddFactorial' o = o * oddFactorial' (o-2)
...
leftpad s i = 
    if length s >= i
        then s
        else lefty (i - length s) s
    
lefty :: Int -> String -> String
lefty 0 s = s
lefty i s = lefty (i-1) (" " ++ s)
...
countdown' 0 s = s
countdown' i s = countdown' (i-1) (s ++ show i ++ "... ")
