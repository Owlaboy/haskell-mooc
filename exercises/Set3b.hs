...
buildList start count end = addToBeginging 0 count start [end]

addToBeginging i count num ls 
    | i == count = ls
    | otherwise = addToBeginging (i+1) count num (num:ls)
...
sumsOf' [] lastSum = []
sumsOf' (x:xs) lastSum = lastSum+x : sumsOf' xs (lastSum+x)
------------------------------------------------------------------------------
...
maybeMap f [] = []
maybeMap f (x:xs) = justifier f (f x) xs

justifier _ (Just a) [] = [a]
justifier _ Nothing [] = []
justifier f (Just a) (x:xs) = a:justifier f (f x) xs
justifier f Nothing (x:xs) = justifier f (f x) xs
