...
totalPrice (MkShoppingEntry name p c) = p * fromIntegral c
...
buyOneMore (MkShoppingEntry name p c) = MkShoppingEntry name p (c+1)
...
data Person = PersonUndefined { name :: String, age :: Int}
  deriving Show
...
setName name p = PersonUndefined name (age p)

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age p = PersonUndefined (name p) age
...
data Position = PositionUndefined { xVal::Int, yVal::Int}
...
up p = PositionUndefined (xVal p) (yVal p +1)

-- right increases the x value of a position by one
right :: Position -> Position
right p = PositionUndefined (xVal p +1) (yVal p)
...
data UpDown = Uppies { val :: Int } | Downies { val :: Int }
...
tick :: UpDown -> UpDown
tick (Uppies v) = Uppies (v+1)
tick (Downies v) = Downies (v-1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (Uppies v) = Downies v
toggle (Downies v) = Uppies v
...
rgb (Mix col1 col2) = mixer (rgb col1) (rgb col2)

rgb (Invert col) = map (1.0 - ) (rgb col)
...
recursor :: (Eq t, Num t) => t -> Nat
recursor 0 = Zero
recursor n = PlusOne (recursor (n-1))
...
fromBin' End _ = 0
fromBin' (O b) d = fromBin' b (d+1)
fromBin' (I b) d = 2^d + fromBin' b (d+1)
